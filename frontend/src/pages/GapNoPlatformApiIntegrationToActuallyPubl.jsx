...
export default function GapNoPlatformApiIntegrationToActuallyPubl() {
  ...
  async function run() {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-platform-api-integration-to-actually-publ', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 900, margin: '0 auto', fontFamily: 'system-ui, sans-serif' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 4 }}>No platform API integration to actually publish (accounts route is registration only)</h1>
      <p style={{ color: '#666', marginBottom: 16 }}>AISocialMediaManager - Gap feature</p>
